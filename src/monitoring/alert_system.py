...
class AlertSystem:
    ...
    async def check_price_alerts(self, symbol: str, price: float) -> List[str]:
        """가격 알림 확인"""
        alerts = []
        if symbol in self.alert_thresholds:
            threshold = self.alert_thresholds[symbol]
            if price >= threshold['upper']:
                alerts.append(f"{symbol} price above {threshold['upper']}")
            elif price <= threshold['lower']:
                alerts.append(f"{symbol} price below {threshold['lower']}")
        return alerts

    async def send_performance_alert(self, metrics: Dict) -> None:
        """성과 알림 전송"""
        if metrics['drawdown'] > self.alert_thresholds.get('max_drawdown', 0.1):
            await self._send_alert(
                f"Warning: Drawdown ({metrics['drawdown']:.2%}) exceeds threshold"
            )
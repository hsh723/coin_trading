...
class IntegratedTrader:
    ...
    def setup_logging(self):
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler('logs/integrated_trader.log'),
    ...
    def check_exit_conditions(self, df: pd.DataFrame) -> bool:
        ...
        # 롱 포지션에서 하락추세로 전환
        if self.current_position['type'] == 'long' and not trend['long_trend_up']:
            self.logger.info("롱 포지션 추세 반전 청산")
            return True
            
        # 숏 포지션에서 상승추세로 전환
        elif self.current_position['type'] == 'short' and trend['long_trend_up']:
        ...
        if holding_time > timedelta(hours=2):
            self.logger.info("최대 보유 시간 초과로 청산")
            return True
    ...
    def run(self):
        """트레이딩 루프를 실행합니다."""
        self.logger.info("통합 매매 트레이더 시작")
